...
class Laporan extends CI_Model
{
    public function getDataPembelian($tglAwal, $tglAkhir)
    {
        $qry = $this->db->query("SELECT  
                                hds.kodepem, DATE(hds.tglpem) as tglpem, hds.supp, kb.nama_barang AS namabrg, dt.satuan, dt.harga_beli AS hrgbeli, dt.qty, dt.qty_sisa, dt.qty_gudang, dt.qty_batal, dt.qty * dt.harga_beli AS 'total',
                                hds.totalqty AS totqty, hds.totalqtysisa AS totqtysisa, hds.totalqtygd AS totqtygd, hds.totalqtybatal AS totqtybatal, hds.totalharga AS totharga, hds.totaltotal AS tottotal
                            ...
                            LEFT JOIN (SELECT hd.kd_pembelian AS kodepem, hd.tgl_pembelian AS tglpem, su.nama_supplier AS supp, 
                                ...
                                (SELECT SUM(dts.qty_sisa)
                                FROM detail_pembelian dts WHERE hd.kd_pembelian = dts.kd_pembelian) AS totalqtysisa,
                                (SELECT SUM(dts.qty_gudang)
                                FROM detail_pembelian dts WHERE hd.kd_pembelian = dts.kd_pembelian) AS totalqtygd,
                                (SELECT SUM(dts.qty_batal)
                                FROM detail_pembelian dts WHERE hd.kd_pembelian = dts.kd_pembelian) AS totalqtybatal,
                                (SELECT SUM(dts.harga_beli)
                                FROM detail_pembelian dts WHERE hd.kd_pembelian = dts.kd_pembelian) AS totalharga,
                                (SELECT SUM(dts.qty * dts.harga_beli)
